...
You can install the released version of utils.add from [CRAN](https://CRAN.R-project.org) with:
...
Some functions facilitate data extraction. These include: 
  ...
  * `match_closest()` -- Extract the values in one dataset that are closest in time to the times in another dataset;
...
Some functions which facilitate learning from data. These include: 
  ...
  * `difference()` -- Compute the difference between two values (numbers or timestamps);
  * `serial_difference()` -- Compute the difference between consecutive pairs of values (numbers or timestamps) in a sequence;
  * `yday2date()` -- Compute the date/month/season from the Julian day;
  * `mmyy()` -- Define (ordered) month-year categories from timeseries data;
...
* `check_input()` -- Check inputs supplied to a function to ensure that they are supported; 
